...
def login_courses(school, username, password):
    ...
    while(1):
        ...
        if login_vercode.status_code == 200:
            ...
            img.show()
            ver_code = int(input(COLOR.NOTE + " please input the vercode:" + COLOR.END))
...
def choose_course(courses_lt):
    ...
    while 1:
        course_id = int(input(COLOR.NOTE + 'please select which course by order(0 is exit):' + COLOR.END))
...
def perform(logindata):
    courses_lt,cookies=login_courses(logindata[0].strip(' \t\n'), logindata[1].strip(' \t\n'), logindata[2].strip(' \t\n'))
